...
#my $CMD = "$hadoop_home/bin/hadoop fs -stat $HDFS_DIR 2>&1";
#println("$CMD");
...
for ($host = 0; $host < $num_hosts; $host++)
{
   ...
   !system qq(scp $ssh_opts gen_and_load.sh   $hosts[$host]:$LOCAL_DIR/.) or die $!;
#   !system qq(scp $ssh_opts tpch_data_gen.zip $hosts[$host]:$LOCAL_DIR/.) or die $!;
   !system qq(scp $ssh_opts tpch_data_gen.tar.gz $hosts[$host]:$LOCAL_DIR/.) or die $!; 
   !system qq(scp $ssh_opts data.properties   $hosts[$host]:$LOCAL_DIR/.) or die $!;
